...
Example:
java -jar swagger-codegen-cli-2.4.0.jar generate -i swagger.yaml -l jaxrs-cxf-client -c config.json -o ./swagger-jaxrs-client-kek/
### LATEST USED VERSION
...
p.s. keep in mind that swagger-codegen tool contain a lot of bugs, so try to use the most recent snapshot, and then double-check result manually.  